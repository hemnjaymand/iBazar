...
export async function GET(): Promise<NextResponse<ApiResponse>> {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        productCount,
        databaseUrl: process.env.DATABASE_URL.replace(/:[^:@]+@/, ":****@"), // نمایش URL بدون رمز
      },
    });
  } catch (error) {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      if (error.code === "P1000") {
        errorMessage =
          "احراز هویت در دیتابیس ناموفق بود. رمز عبور را بررسی کنید.";
      }
      // خطای اتصال به سرور
      else if (error.code === "P1001") {
        errorMessage =
          "اتصال به سرور دیتابیس ممکن نیست. نشانی یا پورت را بررسی کنید.";
      }
      // خطای دسترسی به دیتابیس
      else if (error.code === "P1002") {
        errorMessage =
          "دسترسی به دیتابیس رد شد. ممکن است IP شما در Supabase محدود شده باشد.";
      }
      details = error.meta;
